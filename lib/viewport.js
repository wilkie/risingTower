...
var initViewport = function(RisingTower) {
  var Viewport = RisingTower.Viewport = function(x, y, width, height, world) {
    ...
    this._overlay     = Viewport._phaser.add.graphics(x, y);
    this._texture     = Viewport._phaser.add.renderTexture(width, height);
    this._screen      = Viewport._phaser.add.sprite(x, y, this._texture);
  ...
  Viewport.prototype.mouseDownEvent = function(pointer, localX, localY) {
    this._world.addItem(new RisingTower.Room());

    this.updateOverlay();
  };

  Viewport.preload = function(phaser) {
    Viewport._phaser = phaser;
  };
