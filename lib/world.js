...
var initWorld = function(RisingTower) {
  var World = RisingTower.World = function(phaser) {
    ...
    this._roomGroup     = phaser.make.group(null);
    this._elevatorGroup = phaser.make.group(null);
  ...
  World.prototype.mouseDownEvent = function(pointer, worldX, worldY) {
    ...
    if (pointer.leftButton.duration == 0) {
      this.addItem(RisingTower.Rooms.Office);
    }

    if (pointer.rightButton.duration == 0) {
      this.addItem(RisingTower.Elevators.Normal);
    }
  };
