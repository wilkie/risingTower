var initUserInterface = function(RisingTower) {
  var UserInterface = RisingTower.UserInterface = function(screen) {
    this._screen = screen;
  }
  ...
  UserInterface.create = function(phaser) {
    ...
    phaser.input.onUp.add(UserInterface.mouseUpEvent, this);
  };
  ...
  UserInterface.mouseUpEvent = function(pointer) {
    // Discover the viewport we are on.
    // Adjust the local coordinates to the viewport coordinates
    var x = pointer.x;
    var y = pointer.y;
    ...
    if (viewport) {
      x -= viewport.boundingBox().x;
      y -= viewport.boundingBox().y;
      viewport.mouseUpEvent(pointer, x, y);
    }
  };
